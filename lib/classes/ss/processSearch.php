...
//      No changes to the DB can be made!
    ...
    // Gets entity IDs for retrieving entity data
        if (isset($_POST['iffValue']) == 'undefined' && $_POST['entityValue'] == 'none' && $_POST['ownerValue'] == 'none' && $_POST['systemValue'] == 'none' && $_POST['fromYearValue'] == 'none') {
            echo json_encode(array("status" => "error", "error" => $_SESSION['error']['012']));
        } else {
            $query = "SELECT `entityID` FROM `changesbasic` WHERE ";

            if (isset($_POST['iffValue']) == 'all' || isset($_POST['iffValue']) == 'undefined') {
