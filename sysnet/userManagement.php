<?php
	...
	if (varCheck($_SESSION['userPermissions']['groupAdmin']) == 1) {
...
// Does checks on permissions to keep the tabs displaying properly
	if (varCheck($_SESSION['userPermissions']['owner']) == 1) {
		$tabPercent = 24;
			...
			<div style="float: left; width: 35%" class="textCenter">
				...
				<form method="POST" action="processUsers">
					...
					<select id="newUserPrivs" name="newUserPrivs" class="dropShadow userManagement formField ui-corner-all">
						<?php if (varCheck($_SESSION['userPermissions']['owner']) == 1) { ?>
					...
					<?php if (varCheck($_SESSION['userPermissions']['owner']) == 1 || varCheck($_SESSION['userPermissions']['admin']) == 1) { ?>
